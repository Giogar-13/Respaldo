...
include "./conexion.php";
  ...
  session_start();
  $solicitud = $_GET["consulta"];
  $solicitud = (int)$solicitud;
  $llegadaUsu = $_SESSION['id'];
  $usuario = $llegadaUsu;
  $idPreguntas = [];
  $parciales = [];
  $paqueteFinal = [];
    ...
    if ($solicitud==1 && $encuestas["creador"] != $usuario)
    {
      foreach ($invitados as $key => $value)
      {
        if ($value == $usuario)
        {
          ...
          $consultaHex = "SELECT Nombre FROM USUARIO NATURAL JOIN ENCUESTA WHERE id_usuario='$credorBase' && creador='$credorBase'";
